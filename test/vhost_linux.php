...
$config_replace_params = array(
	'search' => array(
		...
		'192.168.0.197',
		'odbc:TEST1dsn',
		'/usr/local/webdata/51fanli/cache/',
		'sqlsrv:server=mdb.zz.51fanli.it;Database=51fanli',
		", 'param' => array(PDO::SQLSRV_ATTR_ENCODING => PDO::SQLSRV_ENCODING_SYSTEM)",
	),
	'replace' => array(
		'192.168.100.60',
		'192.168.100.60',
...
if(!empty($_POST)) {
	...
	modifiyConf($old_root, $new_root); //修改apache的conf
	modifyConfFile($new_root, $serverName); //修改配置文件
...
function modifiyConf($old_root, $new_root) {
	global $conf;
	$old_root = "\"$old_root\"";
	$new_root = "\"$new_root\"";
	$str = file_get_contents($conf);
	...
	foreach($paths as $path) {
		...
		if(is_file($file)) {
			...
			if(strtoupper(substr(PHP_OS, 0, 3)) != 'WIN') {
				//$str = preg_replace('/,[\s\'"]+param([\S]+)\)/', '', $str);
			}
