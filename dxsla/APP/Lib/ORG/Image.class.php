...
class Image extends Think {//类定义开始
     ...
     * @throws ThinkExecption
      +----------------------------------------------------------
     */
    static public function share_pic($source, $water, $savename=null, $alpha=80, $text = array()) {
        //检查文件是否存在
        ...
        //图像位置,默认为右下角右对齐
        $posY = 100;
        $posX = ($sInfo["width"] - $wInfo["width"]) / 2 + 5;
		...
		//文字开始
		$top = 110 + $wInfo['height'];
		$fontface = dirname(__FILE__) . "/xjlFont.fon";
		foreach($text as $t) {
			$top += 35; 
			$code = mb_convert_encoding($t[4], "html-entities", "utf-8" );
			$fontcolor = imagecolorallocate($sImage, $t[3][0], $t[3][1], $t[3][2]);
			imagettftext($sImage, $t[0], 0, $t[1], $t[2] + $top, $fontcolor, $fontface, $code);
		}
		//文字结束
     ...
     * @return void
      +----------------------------------------------------------
     */
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $cut=0) {
        // 获取原图信息
...
if($cut != 1) {
	...
	if($cut == 2) $scale = $maxWidth / $srcWidth;
	if (($scale >= 1 && $cut==0) || ($maxWidth > $srcWidth && $cut == 2)) {
		// 超过原图大小不再缩略
	...
	$ow = $srcWidth;
	$oh = $srcHeight;
	$k1 = round($height / $width, 2);
	$k2 = round($oh / $ow, 2);
	if($k1 < $k2) {
		$srcWidth = $ow;
		$srcHeight = round($ow * $k1);
		$sw = 0;
		$sh = ($oh - $srcHeight) / 2;
	} else {
		$srcWidth = $oh / $k1;
		...
		$sh = 0;
            ...
            if (function_exists("ImageCopyResampled"))
                ...
                imagecopyresized($thumbImg, $srcImg, 0, 0, $sw, $sh, $width, $height, $srcWidth, $srcHeight);
