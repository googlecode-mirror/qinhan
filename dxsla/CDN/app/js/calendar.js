...
var popUpCal = {
	...
	monthNames: ['一月','二月','三月','四月','五月','六月','七月','八月','九月','十月','十一月','十二月'], // Names of months	dateFormat: 'DMY/', // First three are day, month, year in the required order, fourth is the separator, e.g. US would be 'MDY/'
	...
	speed: '', // Speed of display//medium/closure
	...
	timeSeparators:[' ',':'],	//An array of time separators: the first and second strings are obligatory, while the second and third strings specify//,'AM','PM'
	...
	formatDate: function(day, month, year, hour, minute) {
		...
		$("#show_time").html(dateString.substring(1));
	...
	parseDate: function(Text) {
		...
		if (currentDate.length == 3) {
			...
			if (isNaN(currentDay)) {currentDay = new Date().getDate(); var validDate = false;}
			currentMonth = parseInt(currentDate[this.dateFormat.indexOf('M')],10) - 1;
			if (isNaN(currentMonth)) {currentMonth = new Date().getMonth(); var validDate = false;}
			currentYear= parseInt(currentDate[this.dateFormat.indexOf('Y')],10);
			if (isNaN(currentYear)) {currentYear = new Date().getFullYear(); var validDate = false;}
		} else {
		...
		if (this.timeSeparators && this.timeSeparators.length > 1 && this.timeSeparators[0]) {
			if (currentTime != null && currentTime.length == 2) {
				...
				if (isNaN(currentHour)) {currentHour = new Date().getHours(); var validDate = false;}
			...
			if (isNaN(currentMinute)) {currentMinute = new Date().getMinutes(); var validDate = false;}
	...
	showCalendar: function() {
		...
		var timeSelect = '';
		if (this.timeSeparators && this.timeSeparators.length > 1) {
			//timeSelect += '<div id="calendar_time">';
			if (this.timeSeparators.length == 4) {
			...
			if (this.timeSeparators.length == 4) {
				...
				for (var a = 1; a < 12; a++) {
					if (this.selectedHour == a || (a == 12 && this.selectedHour == 0) || (a + 12 == this.selectedHour)) timeSelect += '<option value="' + a + '" selected="selected">' + a + '时</option>';
					else timeSelect += '<option value="' + a + '">' + a + '</option>';
				}
			} else {
				for (var a = 0; a<24; a++) {
					if (this.selectedHour == a) timeSelect += '<option value="' + a + '" selected="selected">' + a  + '时</option>';
					else timeSelect += '<option value="' + a + '">' + a + '</option>';
				}
			}
			...
			for (var a = 0; a < 59; a++) {
				if (this.selectedMinute == a) timeSelect += '<option value="' + a + '" selected="selected">' +  ((a >= 10) ? a : ('0' + a)) + '分</option>';
		...
		var html = (this.closeAtTop ? '<div id="calendar_control">' +
			...
			'<a id="calendar_prev">' + this.prevText + '</a>' ;
		//alert(timeSelect);
		if (!this.changeYear) {
		...
		html += timeSelect + '<a id="calendar_next">' + this.nextText + '</a>' +'</div><table id="calendar" cellpadding="0" cellspacing="0"><thead>' +
		...
		html += '</tbody></table><!--[if lte IE 6.5]><iframe src="javascript:false;" id="calendar_cover"></iframe><![endif]-->' +
			...
			(this.closeAtTop ? '' : '<div id="calendar_control"><a id="calendar_current">' + this.currentText + '</a>' +'<a id="calendar_close">' + this.closeText + '</a></div>');
		//    <a id="calendar_clear">' + this.clearText + '</a>' +
